// Adapted from https://www.modernescpp.com/index.php/a-lock-free-stack-a-hazard-pointer-implementation/
export module randomstuff:lockfree;
import std;

export namespace LockFreeStack
{
    template <typename T>
    ...
    public:
        ...
        bool isInUse(MyNode* node) 
        {
            for (std::size_t i = 0; i < MaxHazardPointers; ++i) 
            {
                if (HazardPointers<T>[i].pointer.load() == node) 
                    return true;
            }
            return false;
        }
